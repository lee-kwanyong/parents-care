...
export type CarePassportForPlan = {
  id?: string
  elder_name?: string | null
  guardian_name?: string | null
  guardian_phone?: string | null
  hearing_attention?: boolean | null
  mobility_attention?: boolean | null
  allergy_status?: 'none' | 'yes' | 'unknown' | string | null
  has_medications?: boolean | null
  fall_risk_level?: 'low' | 'medium' | 'high' | 'unknown' | string | null
...
export type PlanInput = {
  ...
  carePassport?: CarePassportForPlan | null
}
...
function buildPassportImpact(passport?: CarePassportForPlan | null) {
  if (!passport) {
    return {
      passportApplied: false,
      passportElderName: undefined as string | undefined,
      safetyNotes: [] as string[],
      familyQuestions: [] as string[],
      managerTips: [] as string[],
      opsActions: [] as string[],
      careBundles: [] as CareBundleCode[],
      reassuranceBoost: 'none' as 'none' | 'attention' | 'urgent'
    }
  }

  const safetyNotes: string[] = []
  const familyQuestions: string[] = []
  const managerTips: string[] = []
  const opsActions: string[] = []
  const careBundles: CareBundleCode[] = []

  const elderName = passport.elder_name || undefined
  const bodyConditions = Array.isArray(passport.body_conditions) ? passport.body_conditions : []
  const allergies = Array.isArray(passport.allergies) ? passport.allergies : []
  const medications = Array.isArray(passport.medications) ? passport.medications : []
  const dietNeeds = Array.isArray(passport.diet_needs) ? passport.diet_needs : []

  if (passport.hearing_attention) {
    safetyNotes.push('청력 주의: 설명은 천천히, 잘 들리는 방향에서 진행')
    opsActions.push('매니저에게 청력 주의사항 전달')
  }

  if (passport.mobility_attention) {
    safetyNotes.push('이동 주의: 계단, 장거리 이동, 화장실 이동 시 보조 필요')
    opsActions.push('병원 동선에서 계단·긴 대기·휠체어 위치 확인')
    careBundles.push('hospital_day')
  }

  for (const condition of bodyConditions) {
    if (condition.label) safetyNotes.push(condition.label)
    if (condition.managerTip) managerTips.push(condition.managerTip)
  }

  if (passport.allergy_status === 'yes') {
    safetyNotes.push('알러지 있음: 약·음식·검사 전 보호자 확인 필요')
    familyQuestions.push('알러지 종류와 피해야 할 약·음식이 무엇인가요?')
    managerTips.push('알러지가 있으므로 약·음식·검사 전 반드시 보호자에게 확인하세요.')
    opsActions.push('진료·약국·식사 연결 전 알러지 정보 확인')
  } else if (passport.allergy_status === 'unknown') {
    safetyNotes.push('알러지 유무 확인 필요')
    familyQuestions.push('알러지 유무를 확인해주세요.')
    opsActions.push('알러지 확인 전 약·음식 관련 안내 주의')
  }

  for (const allergy of allergies) {
    if (allergy.memo) safetyNotes.push(`알러지 메모: ${allergy.memo}`)
  }

  if (passport.has_medications || medications.length > 0) {
    safetyNotes.push('복용약 있음: 병원 접수·진료 시 약 정보 확인')
    managerTips.push('복용약이 있으므로 병원 접수·진료 시 약 정보를 확인하세요.')
    opsActions.push('복용약 메모를 진료 리포트와 복약 확인 흐름에 반영')
    careBundles.push('medication_check')
  } else {
    familyQuestions.push('현재 복용 중인 약이 있나요? 약 봉투 사진도 괜찮습니다.')
  }

  for (const medication of medications) {
    if (medication.memo) safetyNotes.push(`복용약 메모: ${medication.memo}`)
  }

  if (dietNeeds.length > 0) {
    safetyNotes.push('식사 제한 있음: 안심밥상·회복식 연결 시 식단 조건 확인')
    managerTips.push('식사 제한이 있으므로 안심밥상·회복식 연결 시 식단 조건을 확인하세요.')
    opsActions.push('안심밥상 연결 시 식사 제한 조건 전달')
    careBundles.push('meal_delivery')
  }

  for (const diet of dietNeeds) {
    if (diet.label) safetyNotes.push(`식사 조건: ${diet.label}`)
  }

  if (passport.fall_risk_level === 'high') {
    safetyNotes.push('낙상 고위험: 이동·화장실·귀가 확인 강화')
    familyQuestions.push('최근 넘어지신 적이 있나요?')
    managerTips.push('낙상 위험이 높으므로 이동 시 팔을 잡기보다 보행 속도를 맞추고 턱·계단을 먼저 안내하세요.')
    opsActions.push('낙상 고위험 플래그로 현장 체크리스트 강화')
  } else if (passport.fall_risk_level === 'medium') {
    safetyNotes.push('낙상 주의: 이동 시 속도 조절 필요')
  } else if (passport.fall_risk_level === 'unknown') {
    familyQuestions.push('최근 넘어지신 적이 있는지 확인해주세요.')
  }

  if (passport.communication_notes) {
    managerTips.push(passport.communication_notes)
    safetyNotes.push(`응대 메모: ${passport.communication_notes}`)
  }

  if (passport.emergency_notes) {
    opsActions.push(`긴급 메모 확인: ${passport.emergency_notes}`)
    safetyNotes.push(`긴급 메모: ${passport.emergency_notes}`)
  }

  const summary = passport.care_summary || {}

  if (Array.isArray(summary.managerTips)) {
    managerTips.push(...summary.managerTips)
  }

  if (Array.isArray(summary.familyQuestions)) {
    familyQuestions.push(...summary.familyQuestions)
  }

  if (Array.isArray(summary.reassuranceWarnings)) {
    safetyNotes.push(...summary.reassuranceWarnings)
  }

  const reassuranceBoost =
    passport.fall_risk_level === 'high' || passport.allergy_status === 'yes'
      ? 'attention'
  ...
  return {
    ...
    passportElderName: elderName,
    safetyNotes: unique(safetyNotes).slice(0, 8),
    familyQuestions: unique(familyQuestions).slice(0, 3),
    managerTips: unique(managerTips).slice(0, 8),
    opsActions: unique(opsActions).slice(0, 8),
    careBundles: Array.from(new Set(careBundles)),
    reassuranceBoost
  }
}
...
export function buildSimpleCarePlan(input: PlanInput): SimpleCarePlan {
  ...
  const passportImpact = buildPassportImpact(input.carePassport)
  ...
  for (const code of passportImpact.careBundles) {
    if (!careBundles.some((item) => item.code === code)) {
      careBundles.push(bundle(code))
    }
  }
  ...
  const primaryActions = unique([
    ...
    ...passportImpact.familyQuestions,
    ...(socialCare ? ['비용 부담이 있으면 공공지원 연결을 함께 확인합니다'] : [])
  ]).slice(0, 3)

  const familyQuestions = unique([
    ...passportImpact.familyQuestions,
    ...questionsByPack[packCode]
  ]).slice(0, 3)

  const opsActions = unique([
    ...passportImpact.opsActions,
    ...opsByPack[packCode],
  ...
  const reassuranceState =
    isUrgent
      ? '긴급'
      : passportImpact.reassuranceBoost === 'attention'
        ? '확인 필요'
        : packCode === 'not_sure_consult'
          ? '확인 필요'
          : '안심'

  const passportSentence =
    passportImpact.passportApplied && passportImpact.safetyNotes.length > 0
      ? ` 케어패스포트 기준으로 ${passportImpact.safetyNotes.slice(0, 2).join(', ')} 항목도 함께 반영했습니다.`
      : passportImpact.passportApplied
        ? ' 케어패스포트도 함께 확인했습니다.'
        : ' 아직 케어패스포트가 없어서 부모님 상태 등록을 권장합니다.'

  return {
    intakeId: input.intakeId,
    reassuranceState,
    title: titleByPack[packCode],
    oneMinuteSummary: `${summaryByPack[packCode] || summaryByPack.not_sure_consult}${passportSentence}`,
    primaryActions,
    familyQuestions,
    careBundles,
    opsActions,
    ...
    socialCareNote: socialCare
      ...
      : undefined,
    nextContactScript:
      '안녕하세요. 부모님 케어 요청을 확인했습니다. 부모님 상태까지 확인해서 필요한 질문은 세 가지만 드리고, 쉬운 케어플랜으로 정리해드리겠습니다.',
    passportApplied: passportImpact.passportApplied,
    passportElderName: passportImpact.passportElderName,
    passportSafetyNotes: passportImpact.safetyNotes,
    passportFamilyQuestions: passportImpact.familyQuestions,
    passportManagerTips: passportImpact.managerTips
  }
}
