    ...
    <main className="min-h-screen bg-slate-50 px-5 py-8 text-slate-900">
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {allCarePacks.map((pack) => (
            <article key={pack.code} className="rounded-3xl bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-base leading-7 text-slate-600">{pack.target}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {pack.includes.map((item) => (
                  <span key={item} className="rounded-full bg-slate-100 px-3 py-2 text-sm font-bold text-slate-700">
                    {item}
                  </span>
                ))}
              </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <Link href="/ops/plan-builder" className="rounded-2xl bg-slate-900 px-5 py-4 font-black text-white">
            운영실 플랜 만들기
          </Link>
