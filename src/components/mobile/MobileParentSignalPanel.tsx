...
const signals: Signal[] = [
  {
    ...
    emoji: '✅',
    title: '괜찮아요',
    desc: '오늘 상태가 괜찮다고 보호자에게 알려요.'
  },
  {
    key: 'meal',
    emoji: '🍚',
    title: '밥을 못 먹었어요',
    desc: '식사 확인과 도움 요청이 필요해요.'
  },
  {
    key: 'medication',
    emoji: '💊',
...
export function MobileParentSignalPanel() {
  ...
  function saveBasicInfo(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    setMessage('기본 정보가 이 화면에 입력되었습니다. 이제 아래 버튼으로 신호를 보낼 수 있습니다.')
  }
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#F7FFFC_0%,#FFFFFF_56%,#F6FBFF_100%)] px-4 py-5 text-[#17443F]">
      <section className="mx-auto max-w-md space-y-4">
        ...
        <form onSubmit={saveBasicInfo} className="rounded-[2rem] bg-white/95 p-5 shadow-sm ring-1 ring-[#D6EDE7]">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <input
              value={guardianPhone}
              onChange={(event) => setGuardianPhone(phoneOnly(event.target.value))}
              inputMode="tel"
              placeholder="보호자 휴대폰 번호"
              className="rounded-2xl border border-[#D6EDE7] bg-white px-4 py-4 text-base font-bold outline-none"
            />
            <input
              value={serviceArea}
              onChange={(event) => setServiceArea(event.target.value)}
              placeholder="권역 예: 청양읍"
              className="rounded-2xl border border-[#D6EDE7] bg-white px-4 py-4 text-base font-bold outline-none"
            />
            <input
              value={addressHint}
              onChange={(event) => setAddressHint(event.target.value)}
              placeholder="주소 힌트, 선택"
              className="rounded-2xl border border-[#D6EDE7] bg-white px-4 py-4 text-base font-bold outline-none"
            />
          </div>
        </form>
          ...
          {signals.map((signal) => (
            ...
            >
              ...
              {loadingKey === signal.key ? (
                <div className="mt-4 rounded-2xl bg-white/70 px-4 py-3 text-center text-sm font-black">
                  보내는 중...
                </div>
              ) : null}
            </button>
