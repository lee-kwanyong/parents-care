...
function severityClass(severity: string) {
  if (severity === 'Red') return 'bg-[#FFF4F4] text-[#8A3030] ring-[#F3C8C8]'
  if (severity === 'Orange') return 'bg-[#FFF9EE] text-[#795C22] ring-[#F3DEB5]'
        ...
        <section className="rounded-[2rem] bg-white p-5 shadow-[0_18px_52px_rgba(49,151,136,0.08)] ring-1 ring-[#D6EDE7] sm:rounded-[2.5rem] sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              disabled={!selected}
              className="rounded-2xl bg-white px-5 py-3 text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7] disabled:opacity-50"
            >
              사건 CSV
            </button>
              ...
              <>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="rounded-full bg-[#FAFFFD] px-3 py-1 text-xs font-black text-[#17443F] ring-1 ring-[#D6EDE7]">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:min-w-64 lg:grid-cols-1">
                    {selected.guardianPhone ? (
                      <a href={`tel:${selected.guardianPhone}`} className="rounded-xl bg-white px-4 py-3 text-center text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7]">
                        보호자 전화
                      </a>
                    ) : null}

                    {selected.parentPhone ? (
                      <a href={`tel:${selected.parentPhone}`} className="rounded-xl bg-white px-4 py-3 text-center text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7]">
                        부모님 전화
                      </a>
                    ) : null}
                  </div>
                ...
                <section className="mt-5 rounded-2xl bg-[#FAFFFD] p-4 ring-1 ring-[#D6EDE7]">
                  ...
                  <textarea
                    ...
                    className="mt-3 min-h-24 w-full rounded-2xl border border-[#D6EDE7] bg-white px-4 py-3 text-sm font-bold outline-none"
                  />
                  ...
                  <div className="mt-3 grid gap-2 md:grid-cols-4">
                    <button onClick={() => post('notifyGuardian')} disabled={loading} className="rounded-xl bg-white px-4 py-3 text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7] disabled:opacity-50">
                      보호자 알림
                    </button>
                    <button onClick={() => post('dispatchProviders')} disabled={loading} className="rounded-xl bg-[#247A71] px-4 py-3 text-sm font-black text-white disabled:opacity-50">
                      도움망 요청
                    </button>
                    <button onClick={() => post('assignOperator', { assignedToName: operatorName || '운영실' })} disabled={loading} className="rounded-xl bg-white px-4 py-3 text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7] disabled:opacity-50">
                      담당자 배정
                    </button>
                    <button onClick={() => post('recordContact', { contactType, resultStatus: contactStatus })} disabled={loading} className="rounded-xl bg-white px-4 py-3 text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7] disabled:opacity-50">
                      통화 기록
                    </button>
                    <button onClick={() => post('addNote')} disabled={loading || !note.trim()} className="rounded-xl bg-white px-4 py-3 text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7] disabled:opacity-50">
                      메모 저장
                    </button>
                    <button onClick={() => post('markInProgress')} disabled={loading} className="rounded-xl bg-white px-4 py-3 text-sm font-black text-[#17443F] ring-1 ring-[#D6EDE7] disabled:opacity-50">
                      확인 중
                    </button>
                    <button onClick={() => post('markCompleted')} disabled={loading} className="rounded-xl bg-[#247A71] px-4 py-3 text-sm font-black text-white disabled:opacity-50">
                      완료
                    </button>
                    <button onClick={() => post('cancelRequest')} disabled={loading} className="rounded-xl bg-[#FFF4F4] px-4 py-3 text-sm font-black text-[#8A3030] ring-1 ring-[#F3C8C8] disabled:opacity-50">
                      취소
                    </button>
                  </div>
                </section>
                ...
                <section className="mt-5">
                  ...
                  <div className="mt-5 space-y-3">
                    {selected.timeline.length === 0 ? (
                      ...
                      selected.timeline.map((event) => (
                        <article key={event.id} className="grid gap-3 rounded-2xl bg-[#FAFFFD] p-4 ring-1 ring-[#D6EDE7] sm:grid-cols-[8rem_1fr]">
                          <div>
                            <div className="rounded-full bg-white px-3 py-1 text-center text-xs font-black text-[#2AA897] ring-1 ring-[#D6EDE7]">
                              {kindLabel(event.kind)}
                            </div>
