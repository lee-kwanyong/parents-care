    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <Card>
        ...
        <form action={createWorryRequestFormAction} className="space-y-6">
          <fieldset>
            <legend className="mb-3 text-sm font-black text-slate-700">1. 걱정 선택</legend>
            <div className="grid gap-3 sm:grid-cols-2">
              {categories.map((key) => (
                <label key={key} className="cursor-pointer rounded-3xl border border-slate-200 bg-white p-5 text-left transition hover:border-care-500 hover:bg-care-50 has-[:checked]:border-care-600 has-[:checked]:bg-care-50">
          ...
          <fieldset>
            <legend className="mb-3 text-sm font-black text-slate-700">2. 편한 접수 방식</legend>
            <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-black text-slate-700">3. 상황을 한 줄만 적어주세요</span>
            <textarea
              name="memo"
              required
              rows={5}
              placeholder="예: 어머니가 다음 주 정형외과 예약이 있는데 저는 못 갑니다. 약이랑 보험서류도 챙겨주세요."
              className="w-full rounded-3xl border border-slate-200 bg-white p-4 text-base outline-none focus:border-care-500 focus:ring-4 focus:ring-care-100"
            />
          </label>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <input name="contactName" className="w-full rounded-2xl border border-slate-200 p-3 outline-none focus:border-care-500" placeholder="예: 이관용" />
            </label>
            <label className="block">
              <span className="mb-2 block text-sm font-black text-slate-700">연락처</span>
              <input name="contactPhone" className="w-full rounded-2xl border border-slate-200 p-3 outline-none focus:border-care-500" placeholder="예: 010-0000-0000" />
            </label>
          </div>
          ...
          <button className="w-full rounded-3xl bg-care-600 px-6 py-5 text-xl font-black text-white shadow-soft hover:bg-care-700">
            부모님 걱정 맡기기
          </button>
        </form>
      </Card>
