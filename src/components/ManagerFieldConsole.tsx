...
export function ManagerFieldConsole({ mode = 'manager' }: { mode?: 'manager' | 'ops' }) {
  ...
  const [managerProfiles, setManagerProfiles] = useState<ManagerProfileOption[]>([])
              ...
              {managerProfiles.length === 0 ? (
                ...
                managerProfiles.map((profile) => (
                  <option key={profile.id} value={profile.id}>
                    {profile.manager_name} · {profile.trust_level} · {profile.identity_verified ? '본인확인 완료' : '미완료'}
                  </option>
                ))
              )}
            </select>
