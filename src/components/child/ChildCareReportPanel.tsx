...
function toneClass(tone: Tone) {
  ...
  if (tone === 'good') return 'bg-[#EFFFFA] text-[#2AA897] ring-[#CDEFE7]'
...
function priorityClass(priority: string) {
  if (priority === '높음') return 'bg-[#FFF4F4] text-[#8A3030] ring-[#F3C8C8]'
  if (priority === '중간') return 'bg-[#FFF9EE] text-[#795C22] ring-[#F3DEB5]'
  return 'bg-[#EFFFFA] text-[#2AA897] ring-[#CDEFE7]'
}
...
function MetricCard({
  ...
  return (
    <article className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-[#D6EDE7]">
      ...
      <div className="mt-2 text-4xl font-black tracking-[-0.08em] text-[#17443F]">{value}</div>
...
function FingerprintCard({
  ...
  return (
    <article className="rounded-[2rem] bg-[#FAFFFD] p-5 ring-1 ring-[#D6EDE7]">
      <div className="text-sm font-black text-[#7A9692]">{title}</div>
      <h3 className="mt-2 text-2xl font-black tracking-[-0.06em] text-[#17443F]">
        {item.label}
      </h3>
        ...
        {report ? (
          <>
            ...
            <section className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-[#D6EDE7] sm:p-6">
              ...
              <div className="mt-5 grid gap-3">
                {report.insights.map((insight) => (
                  <div key={insight} className="rounded-2xl bg-[#FAFFFD] p-4 text-sm font-bold leading-7 text-[#637B76] ring-1 ring-[#D6EDE7]">
                    {insight}
                  </div>
                ))}
              </div>
            ...
            {report.sections.map((section) => (
              <section key={section.key} className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-[#D6EDE7] sm:p-6">
            ...
            <section className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-[#D6EDE7] sm:p-6">
              ...
              <div className="mt-5 space-y-3">
                {report.history.map((day) => (
                  <article key={day.date} className="rounded-2xl bg-[#FAFFFD] p-4 ring-1 ring-[#D6EDE7]">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <h3 className="text-lg font-black">{day.date}</h3>
                      <div className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#637B76] ring-1 ring-[#D6EDE7]">
                        일일 점수 {day.score}
                      </div>
                    </div>
