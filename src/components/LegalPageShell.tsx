  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#F7FFFC_0%,#FFFFFF_56%,#F6FBFF_100%)] px-5 py-8 text-[#17443F]">
      <section className="mx-auto max-w-5xl">
        ...
        <section className="mt-5 rounded-[2rem] bg-[#247A71] p-5 text-white sm:p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Link href="/contact" className="rounded-2xl bg-white px-5 py-4 text-sm font-black text-[#17443F]">
              문의하기
            </Link>
...
export function LegalList({ items }: { items: string[] }) {
  return (
    <ul className="space-y-3">
      {items.map((item) => (
        <li key={item} className="rounded-2xl bg-[#FAFFFD] p-4 text-sm font-bold leading-7 text-[#5F7772] ring-1 ring-[#D6EDE7]">
          {item}
        </li>
      ))}
    </ul>
