...
create index if not exists idx_care_response_requests_ops
...
create index if not exists idx_care_response_matches_provider
  on public.care_response_matches(provider_id, match_status);
...
create index if not exists idx_care_providers_ops
  on public.care_providers(provider_type, available_status, service_area);
...
create index if not exists idx_notification_outbox_ops
  on public.notification_outbox(status, reason, created_at desc);
