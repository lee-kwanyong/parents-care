...
alter table public.care_providers add column if not exists fast_dispatch_enabled boolean default true;
alter table public.care_providers add column if not exists current_lat double precision;
alter table public.care_providers add column if not exists current_lng double precision;
alter table public.care_providers add column if not exists service_radius_km integer default 5;
alter table public.care_providers add column if not exists last_seen_at timestamptz;

create index if not exists idx_care_providers_fast_dispatch
  on public.care_providers(provider_type, available_status, verified_status, service_area, fast_dispatch_enabled);
...
create index if not exists idx_care_response_requests_fast_dispatch
  on public.care_response_requests(request_type, risk_level, status, service_area, created_at desc);
...
create index if not exists idx_care_response_matches_dispatch
  on public.care_response_matches(request_id, provider_id, match_status, created_at desc);
