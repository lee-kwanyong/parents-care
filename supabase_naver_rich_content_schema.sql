...
alter table public.naver_content_packages
  add column if not exists package_type text not null default 'naver_blog',
  add column if not exists image_brief jsonb not null default '{}'::jsonb,
  add column if not exists video_brief jsonb not null default '{}'::jsonb,
  add column if not exists notes text,
  add column if not exists updated_at timestamptz not null default now();

alter table public.naver_keyword_research
  add column if not exists insights jsonb not null default '{}'::jsonb;

create index if not exists idx_naver_keyword_research_created_at
on public.naver_keyword_research(created_at desc);

create index if not exists idx_naver_content_packages_created_at
on public.naver_content_packages(created_at desc);

create index if not exists idx_naver_content_packages_status
on public.naver_content_packages(status);

create index if not exists idx_naver_ad_plans_created_at
on public.naver_ad_plans(created_at desc);
